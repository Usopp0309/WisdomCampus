...
public class BluetoothDataServiceImpl extends BaseService implements BluetoothDataService {
	...
	public Pages<BluetoothData> getPageStatisticsBluetoothData(int page, int pageSize, Map<String, Object> paramMap) {
		...
		pages.setPageSize(pageSize);
		@SuppressWarnings("unchecked")
		List<BluetoothData> list = this.getBaseDao().selectListByObject("BluetoothData.getStatisticsBluetoothData",
				paramMap);
		/**
		 * 如果总页数为0，当前页也应该为0
		 */
	...
	public List<BluetoothData> getGroupBluetoothDataByTimeId(Map<String,Object> map) {
		...
		return this.getBaseDao().selectListByObject("BluetoothData.getGroupBluetoothDataByTimeId",
				map);
	}
	...
	public List<BluetoothData> getByClazzAndTimeId(Map<String, Object> map) {
		return this.getBaseDao().selectListByObject("BluetoothData.getByClazzAndTimeId", map);
	}
	/**
	 * 根据班级获得平均值
	 * @author chenyong
	...
	return this.getBaseDao().selectListByObject("BluetoothData.getClazzAvgByClazzIdAndTimeId", map);
	}
}
