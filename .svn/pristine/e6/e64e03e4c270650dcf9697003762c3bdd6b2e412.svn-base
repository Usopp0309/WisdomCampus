...
public class ImportController extends BaseController
{
		...
		try
		{
			...
			if(!CollectionUtils.isEmpty(importDetailList)){
				...
				for(ImportDetail importDetail : importDetailList){
					...
					String roleCode = importDetail.getRoleCode();
					if(userId != null){
						try{
							//判断该用户是不是多学校
							//1.多学校的情况下只删除在本校的关系
	...
	@RequestMapping(value="/tmp.do",method=RequestMethod.POST)
			...
			HttpSession session)
	{
		...
		try
		{
			DBContextHolder.setDBType("3509020027");
			//获取所有导入成功的家长ID
			List<ImportDetail>  sucessList = importService.getSuccessImportDetailParent();
			Map<String, List<String>> map = new HashMap<String, List<String>>();
			for(ImportDetail id : sucessList){
				String key = id.getFamilyId();
				if(map.containsKey(key)){
					List<String> list = map.get(key);
					list.add(id.getParentId());
				}else{
			...
			List<ImportDetail>  nOUserIdList = importService.getNOUserIdImportDetailParent();
			for(ImportDetail id : nOUserIdList){
				//循环更新
				List<String> parentIdList  =  map.get(id.getFamilyId());
				if(!CollectionUtils.isEmpty(parentIdList)){
					String parentId = parentIdList.get(0);
					System.out.println("家长ID:" + id.getUserId()  +" , " + "家长姓名： "+id.getRealName() +"," + "真实家长ID: " + parentId);
					importService.updateImportDetail(Integer.valueOf(parentId), id.getId());
