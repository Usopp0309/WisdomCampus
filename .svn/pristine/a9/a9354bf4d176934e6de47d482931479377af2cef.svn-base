...
<!-- 								<ul class="nav nav-tabs" id="userTab"> -->
<%-- 									<c:forEach items="${sessionScope.user.roleList}" var="roleList"> --%>
<%-- 										<c:if test="${roleList.roleCode eq 'parent'|| roleList.roleCode eq 'president' || roleList.roleCode eq 'classLeader'}"> --%>
<!-- 											<li -->
<%-- 												class="roleLi <c:if test="${roleList.roleCode eq crtCode}">active</c:if> "> --%>
<!-- 												<a -->
...
<script type="text/javascript">
    	  ...
    	  $.ajax({
				...
				success: function(data){
	    			...
	    			for(var i=0;i<list.length;i++){
	    				...
	    				html+=' <td><a class="btn btn-success btn-sm" onclick="showData(this,'+list[i].clazzId+','+list[i].bluetoothCourseTimeId+');">详情</a>';
	    				html+=' <a class="btn btn-success btn-sm" onclick="compareClazz(\''+list[i].courseName+'\',\''+list[i].startTime+'-'+list[i].time+'\');">班级与班级</a></td><tr>';
	    			}
	    		    }
	    		    $("#tbody").html(html);
				}
      ...
      function showData(obj,clazzId,bluetoothCourseTimeId){
    	   $("#tbody tr[name='children']").each(function(){
    		   $(this).remove();
    	   });
    	  $.ajax({
