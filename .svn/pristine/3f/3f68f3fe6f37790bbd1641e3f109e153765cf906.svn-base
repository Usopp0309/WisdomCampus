								...
								<div class="tab-pane active" id="tab_4">
									...
									<div class="portlet box green"  >
										...
										<div class="portlet-title">
											<div class="caption"><i class="icon-user"></i>地点管理</div>
                                            <div class="tools">
...
<!--										<div class="portlet-body" style="height: 35px">-->
<!--											-->
<!--											 查询表单begin -->
...
<!--									    </div>-->
								    ...
								    <div class="actions" style="margin-top: -20px;margin-bottom: 7px">
										...
										<a href="#" class="btn green" id="addPlace" style="display:none;">
	...
	<script>
		...
		jQuery(document).ready(function() {    
			...
			loadPlaceList(null, userId, roleId, null);
			...
			$(".roleTab").click(function(){
				...
				loadPlaceList(name, userId, roleId, null);
			...
			$("#addPlace").click(function(){
				var userId = $("ul li.active").find('a').attr("attr1");	
				var roleId = $("ul li.active").find('a').attr("attr2");
				standardPost('<%=basePath%>place/toAddPlace.do',{userId:userId,roleId:roleId})
			});
			...
			$("#search").click(function(){
				...
				loadPlaceList(queryContent, userId, roleId, null);
			});
		...
		//加载记录点页面
		function loadPlaceList(queryContent, userId, roleId, cPage)
		{
			var body = $("#body");
			App.blockUI(body);
			...
			$.ajax({
				type: "post",
				url: "<%=basePath%>place/loadPlaceList.do",
				data:{
				...
				success: function(data){
					...
					$.each(page.list, function(n, value) {  
            			...
            			appendHtml += '<td width="30%">' + time + '</td>';
            			appendHtml += '<td width="20%"><a href="javascript:;" class="edit" onclick="addPosition(' + value.id + ')">增加设备</a></td>';
            			appendHtml += '<td width="20%"><a href="javascript:;" class="edit" onclick="viewingPosition(' + value.id + ')">查看设备</a></td>';
           				appendHtml += '</tr>';
          			}); 
		...
		//点击增加设备
		function addPosition(placeId)
		{
			var userId = $("ul li.active").find('a').attr("attr1");	
			var roleId = $("ul li.active").find('a').attr("attr2");
			standardPost('<%=basePath%>position/toAddPosition.do',{userId:userId,roleId:roleId,placeId:placeId});
		}
		
		//点击查看设备
		function viewingPosition(placeId)
		{
			var userId = $("ul li.active").find('a').attr("attr1");	
			var roleId = $("ul li.active").find('a').attr("attr2");
			standardPost('<%=basePath%>place/toViewingPosition.do',{roleId:roleId,placeId:placeId})
		}
		...
		function jumpPage(type)
		{
			...
			loadPositionList(null, userId, roleId, cPageInt);
