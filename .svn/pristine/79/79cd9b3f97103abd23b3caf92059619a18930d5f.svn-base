...
<head>
	...
	<!--公共css结束-->
	<style type="text/css">
	  .select_role dl{top:40px;}
	  .select_role dl dd:nth-child(1){bottom:50px;}
	</style> 
</head>
...
<body>
			      ...
			      <c:forEach items="${patrolList}" var="p" varStatus="ls">
			          <c:choose>
			              ...
			              <c:otherwise>
							...
							<td><a href="#" onclick="toViewMap(${p.placeId},${p.userId })">${p.realName }</a></td>
							<td>
	...
	<div class="select_role">
		<dl>
			...
			<r:right rightCode="patrolModify">
				<dd class="glyphicon glyphicon-map-marker" id="setPlace"></dd>
			</r:right>
		</dl>
	...
	$(function(){
		$("#search").on("click",function(){
			...
			standardPost('<%=basePath%>patrol/toPatrolSearchApp.do',{apiKey:apiKey,schoolId:schoolId,createDate:createDate});
			});

		$("#setPlace").on("click",function(){
			var apiKey = $("#apiKey").val();
			var schoolId = $("#schoolId").val();
			standardPost('<%=basePath%>patrol/toPatrolPlaceListApp.do',{apiKey:apiKey,schoolId:schoolId});
			});
	});	
		...
		function loadMoreList()
		{
			...
			$.ajax({
				...
				success: function(data){
					...
					$.each(data.patrolList, function(n, value) {  
						...
						appendHtml += '<td><a href="#" onclick="toViewMap('+value.placeId+','+value.userId+')">'+value.realName+'</a></td>';
	   	...
	   	function toViewMap(placeId,userId){
	   		var createDate = $("#createDate").val();
	   		standardPost('<%=basePath%>patrol/toViewMapApp.do',{userId:userId,placeId:placeId,createDate:createDate});
	   	}
