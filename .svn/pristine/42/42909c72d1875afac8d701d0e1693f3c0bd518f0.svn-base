...
public class YinxinAndWisdomExchangeServiceImpl extends BaseService implements YinxinAndWisdomExchangeService
{
			...
			Long schoolId)
	{
		...
		User pushItem = commonService.getUserByUserIdForPush(userId);
		if(pushItem!=null){//IOS端检测用户需要发送消息
		...
		pi.PushContent = "系统监测到您是智慧校园用户,是否设置并登录";
		pi.PushType = PUSHTYPE.YIXINTOWISDOM.getType();
		pi.PushContentType = PUSHTYPE.YIXINTOWISDOM.getContentType();
		pi.title = PUSHTYPE.YIXINTOWISDOM.getName();
