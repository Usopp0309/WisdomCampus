...
public class MeetingController extends BaseController
{
	...
	public String doAddMeeting(HttpServletRequest request, HttpServletResponse response, HttpSession session, ModelMap modelMap)
	{
		try
		{
		...
		// 接受前台选中教师或部门
		String strs = request.getParameter("selectedObj");
		String content = request.getParameter("content");
		Integer type = Integer.parseInt(request.getParameter("type"));
		String[] receiverArr = strs.split(",");
		...
		try
		{
			try
			{
				...
				map.put("receiverArr", receiverArr);
			} catch (Exception e)
			{
				f = 1;
				e.printStackTrace();
	 ...
	 * 推送会议信息
	 */
	@RequestMapping(value = "/sendMeetingMsg.do")
	public String sendMeetingMsg(HttpServletRequest request, HttpSession session, ModelMap modelMap)
	{
		User user = (User) session.getAttribute("user");
		...
		List<MeetingDetail> details = meetingDetailService.selectMeetingDetailByMeetingId(meetingId);

		// set集合用户保存所有接收者信息
		Set<User> setUser = new HashSet<User>();
		for (MeetingDetail detail : details)
		{
			User teacher = commonService.getUserDetailByUserId(detail.getAttendeeId());
			setUser.add(teacher);
		}
		Meeting meeting = meetingService.selectMeetingByMeetingId(meetingId).get(0);
		...
		final String meetingNameFinal = meeting.getName();
		final String placeNameFinal = meeting.getPlaceName();
		final School schoolFinal = school;
		final String startTimeFinal = meeting.getStartTime();
			...
			public void run()
			{
				try
				{
					...
					for (User attendee : receiverListFinal)
					{
						...
						{
							// 1只发短信 3发短信和推送
							if (informationTypeFinal.getType() == 1 || informationTypeFinal.getType() == 3)
							{
								...
								String sendContent = attendee.getRealName() + "老师您好,请您于" + startTimeFinal + "前,到学校参加“" + meetingNameFinal + "”会议，会议地点：“" + placeNameFinal
										+ "”,请及时参加。如有问题请及时和会议组织者" + user.getRealName() + "联系。" + schoolFinal.getSchoolName();
								...
								if (schoolHasMessage)// 学校有短信套餐
								{
									...
									{
										// 调用短信接口
										boolean flag = commonService.sendSmsByDB(user.getSchoolId(), PUSHTYPE.MEETINGPUSH, attendee.getPhone(), attendee.getUserId(), sendContent,
												null, informationTypeFinal.getType());
										if (flag)
										{
									...
									// 虚拟套餐关闭
									{
										if (isSchoolSingleMsg)
										{
																						...
																						// 直接发送
											{
												// 调用短信接口
												// 调用短信接口
												boolean flag = commonService.sendSmsByDB(user.getSchoolId(), PUSHTYPE.MEETINGPUSH, attendee.getPhone(), attendee.getUserId(),
														sendContent, null, informationTypeFinal.getType());
												if (flag)
												{
													smsService.saveSmsDetail(sendContent, attendee, attendee, 0, "3");
												} else
	...
	public String toEditMeeting(HttpServletRequest request, HttpSession session, ModelMap modelMap)
	{
		...
		// 会议的id
		Integer meetingId = Integer.valueOf(request.getParameter("meetingId"));
		List<Meeting> meets = meetingService.selectMeetingByMeetingId(meetingId);

		modelMap.addAttribute("user", user);
		modelMap.addAttribute("meeting", meets.get(0));
		modelMap.addAttribute("meets", meets);
	 ...
	 * 进入编辑会议页面 20151208
	 */
	@RequestMapping(value = "/deleteMeetingById.do")
	public String deleteMeetingById(HttpServletRequest request, HttpServletResponse response, HttpSession session, ModelMap modelMap)
	{
		response.setCharacterEncoding("utf-8");
		try
		{
			...
			try
			{
				Integer meetingId = Integer.valueOf(request.getParameter("id"));
	...
	public String doEditMeeting(HttpServletRequest request, HttpServletResponse response, HttpSession session, ModelMap modelMap)
	{
		try
		{
			request.setCharacterEncoding("UTF-8");
		...
		User user = (User) session.getAttribute("user");
		// 会议名称
		String name = request.getParameter("name"); // 得不到值
		// 会议地点
		String place = request.getParameter("place");
		// 开始时间
		String startTime = request.getParameter("startTime");
		...
		String strs = request.getParameter("selectedObj");
		String content = request.getParameter("content");
		Integer inoutType = StringUtil.toint(request.getParameter("inoutType"));
		String[] receiverArr = strs.split(",");
		...
		meeting.setUserId(user.getUserId());
		meeting.setContent(content);
		meeting.setInoutType(inoutType);
	...
	public String selectMeetingdetail(HttpServletRequest request, HttpServletResponse response, HttpSession session, ModelMap modelMap)
	{
		...
		Integer meetingId = Integer.valueOf(request.getParameter("meetingId"));
		Meeting meetingdetail = meetingService.selectMeetingByMeetingId(meetingId).get(0);
