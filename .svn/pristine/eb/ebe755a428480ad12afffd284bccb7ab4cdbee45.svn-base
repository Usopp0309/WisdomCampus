...
public class CameraController extends BaseController
{
	...
	public String detailCamera(Integer cameraId,String roleCode, ModelMap modelMap)
	{
		Camera camera = cameraService.getCameraById(cameraId);
		modelMap.addAttribute("camera", camera);
	 ...
	 * 增加摄像头和班级的关系
	 */
	@RequestMapping(value = "/deleteCameraRelation.do")
	public String deleteCameraRelation(HttpServletResponse response,int userOrclazzId,int type,int cameraId)
	{
		response.setCharacterEncoding("UTF-8");
		...
		try
		{
			...
			try
			{
				...
				List<CameraRelation> cameraRelationList = new ArrayList<CameraRelation>();
				if(type==0){
					paramMap.put("clazzId", userOrclazzId);
					CameraRelation cameraRelation = cameraService.getCameraByClazzId(paramMap);
					cameraRelationList.add(cameraRelation);
				}else{
					paramMap.put("userId", userOrclazzId);
					CameraRelation cameraRelation = cameraService.getCameraUserByUserId(paramMap);
					cameraRelationList.add(cameraRelation);
				}
				cameraService.deleteCameraRelation(type, cameraRelationList);
				flag = true;
			} catch (Exception e)
			{
				flag = false;
	...
	public String doSaveUserCamera(HttpServletResponse response, HttpServletRequest request, HttpSession session, ModelMap modelMap)
	{
		...
		for (int a = 0; a < cameraArr.length; a++)
		{
			...
			for (int i = 0; i < userArr.length; i++)
			{
				...
				{
					// 获取部门ID;
					String departmentId = userArr[i].substring(1);
					...
					for (User teacher : teacherList)
					{
						CameraRelation cameraRelation = new CameraRelation();
						cameraRelation.setCameraId(cameraId);
						cameraRelation.setUserId(teacher.getUserId());
						cameraRelation.setCreateTime(TimeUtil.getInstance().now());
						cameraRelationList.add(cameraRelation);
					}
					
				}else if ("S".equals(firstChar) || "T".equals(firstChar))
				{
					// 获取接受學生ID 也就是userid
	...
	public String getCameraListByApiKey(HttpServletRequest request, HttpServletResponse response, HttpSession session)
	{
		...
		try
		{
			...
			if (loginUser != null)
			{
				...
				boolean hasPermission = userService.getLoginUserHasPermission("camera", loginUser);
