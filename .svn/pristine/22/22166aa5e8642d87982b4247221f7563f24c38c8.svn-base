			...
			$.ajax({
				...
				success: function(data){
					...
					$.each(page.list, function(n, value) {  
            			...
            			appendHtml += '<td><r:right rightCode="teacherDelete"><a href="javascript:;" onclick="modifyTeacher(' + value.userId + ',\'' + value.roleCode + '\',' + value.departmentId +')" class="btn btn-sm update_btn">修改</a></r:right><c:if test="${sessionScope.user.type != 2}"><r:right rightCode="teacherModify"><a href="javascript:;" onclick="delTeacher(' + value.userId + ')" class="btn btn-sm delete_btn">删除</a></r:right></c:if></td>';
           				appendHtml += '</tr>';
          			}); 
