...
<head>
	            ...
	            <div class="page-content m_overflow_hidden m_page_content">
	                ...
	                <div class="page-bar m_margin_0_0_0_0">
	                    <ul class="page-breadcrumb">
	                        ...
	                        <li><span>学生考勤补录列表</span></li>
	                    </ul>
	                </div>
	                <h3 class="page-title">学生考勤补录</h3>
	                <div class="row">
	                    <div class="col-md-12 col-sm-12">
	                    	<div class="m_margin_0_0_15_0">
	                    		<!-- 角色切换开始 -->
	                    		<ul class="nav nav-tabs">
	                                <li><a href="<%=basePath %>check/toAttendanceRecord.do">教师补录</a></li>
									<li class="active"><a target="_self">学生补录</a></li>
	                            ...
	                            <div class="portlet-body flip-scroll">
	                                <table class="table table-bordered table-hover">
	                                    <thead class="flip-content">
	                                        <tr>
	                                            <th>班级</th>
												<th class="hidden-sm hidden-md">学号</th>
												<th>姓名</th>
												<th>状态</th>
												<th>备注</th>
												<th>更新时间</th>
	...
	<script>
		...
		jQuery(document).ready(function() { 
			...
			//初始化年级select
			loadAllClassList(userId, 'attendanceRecordManager','teacher');
	
			$("#search").click(function(){
				var userId = $("#userId").val();
				var clazzId = $("#gradeSelect").val();
				var queryContent = $("#queryContent").val();
				loadStudentCheckList(clazzId, queryContent, userId, 'teacher', 1);
			});
				...
				success: function(data){
					...
					$.each(page.list, function(n, value) {  
           				...
           				if (value.remark == null)
						{
							...
							if(value.inoutTime == null){
								appendHtml += '<td>无刷卡</td>';
							}else{
								appendHtml += '<td>' + value.inoutTime + '</td>';
							}
						}
						else
						{
            			...
            			appendHtml += '<td><c:if test="${sessionScope.user.type != 2}"><a href="javascript:;" class=" btn btn-sm green"  onclick="updateStatus(' + j + ',' + value.uId + ','+ value.status + ',\''+ value.updateTime +'\')">保存</a></c:if></td>';
           				appendHtml += '</tr>';
           				j++;
          			}); 
				...
				$.ajax({
					...
					success: function(data){
						...
						if(data=="1"){
							...
							loadStudentCheckList(clazzId, queryContent, userId,roleCode,cPage);
						}else{
		...
		function jumpPage(type)
		{
			...
			var clazzId = $("#gradeSelect").val();
			var queryContent = $("#queryContent").val();
			loadStudentCheckList(clazzId, queryContent, userId,'teacher',cPageInt);
