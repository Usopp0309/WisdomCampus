                                    ...
                                    <form action="#" class="form-horizontal">
                                        <div class="form-body">
                                            <div class="form-group">
                                                <label class="control-label col-md-3">对象</label>
                                            ...
                                            <div class="form-group">
                                                ...
                                                <div class="col-md-9 col-lg-8">
                                                    <div class="personnels" style="display:block">
														<dl>
															<dd onclick="groupClick(this,'depart')" style="text-indent: 0px" class="checked">部门（作用于老师）</dd>
															<dd style="text-indent: 0px" onclick="groupClick(this,'clazz')">班级（作用于学生）</dd>
														</dl>  
													</div>
													<div id="departGroup" class="personnels" style="display:block">
														...
														<dl id="departmentList" style="height:265px;">
														</dl>
													</div>
						...
						$.each(subMenuList, function(y, subMenu) {
							...
							if(subPermissionList.length > 0){
								if(subMenu.hasPermission){
									subMenuAppendHtml+='<dd class="subMenuDD" attr="'+subMenu.permissionCode+'"><i onclick="checkBoxClick(this,\''+subMenu.permissionCode+'\',true)" class="checkbox fa fa-check" attr="'+subMenu.permissionCode+'"></i><span onclick="subMenuClick(\''+subMenu.permissionCode+'\',this)">'+subMenu.name+'</span><em class="next_menu fa fa-chevron-right"></em></dd>';
								}else{
									subMenuAppendHtml+='<dd class="subMenuDD" attr="'+subMenu.permissionCode+'"><i onclick="checkBoxClick(this,\''+subMenu.permissionCode+'\',true)" class="checkbox" attr="'+subMenu.permissionCode+'"></i><span onclick="subMenuClick(\''+subMenu.permissionCode+'\',this)">'+subMenu.name+'</span><em class="next_menu fa fa-chevron-right"></em></dd>';
								}
							}else{
								if(subMenu.hasPermission){
		...
		function save(){
			...
			var l_=layer.confirm('确认提交修改后的权限？', {
				  btn: ['确定','取消'] //按钮
					...
					//获取所有选中的二级菜单
					var subMenuCheckedArray = $(".controls .subMenu").find(".fa-check");
					var subMenuMap = {}; 
					for(var i=0;i<subMenuCheckedArray.length;i++){
						...
						for(var j=0;j<subPermissionArray.length;j++){
							if($(subPermissionArray[j]).hasClass("fa-check")){
