	            ...
	            <div class="page-content m_overflow_hidden m_page_content">
	                ...
	                <div class="row">
	                    <div class="col-md-12 col-sm-12">
	                        ...
	                        <r:right rightCode="addCurriculum">
									<button class="btn btn-default" type="button" id="addCurriculum">创建课程表</button>
										<c:if test="${times!=0 }">
		                        ...
		                        <r:right rightCode="editCurriculum">
								   ...
								   <c:if test="${times!=0 }">
								   		<button class="btn btn-default" type="button" id="editCurriClours">编辑班级课程表</button>
								   </c:if>
								</r:right>
...
<!-- 	                         	style="display: none; -->
	                         	<div class="col-md-4 col-sm-4 col-lg-3 m_margin_top-15" ">
	                         		...
	                         		<input type="text" class="m_input form-control" name="startDate" id="startDate" onchange="searchData();" />
	                         	</div>
<!-- 	                         	style="display: none; -->
	                         	<div class="col-md-4 col-sm-4 col-lg-3 m_margin_top-15" ">
	                         		<span class="m_span">结束时间：</span>
	                         		<input type="text" class="m_input form-control" name="endDate" id="endDate" onchange="searchData();"/>
	                         	</div>
	                         	<div class="col-md-4 col-sm-4 col-lg-3 m_margin_top-15">
	                         		<button type="submit" class="btn green" id="search" onclick="searchData();">查找</button>
	                         	</div>
	                         </div>
	                         </form>
	                        </div>
	                       	<div class="portlet box green m_margin_15_auto_0">
								...
								<tr>
									<td class="bg m_width_10">阶段</td>
									<td class="bg m_width_20">起始时间</td>
									<c:forEach items="${weekList}" var="week">
										<td class="m_width_10">${week.weekName}</td>
									</c:forEach>
								</tr>
							<c:if test="${times!=0 }">
							...
							<c:if test="${times==0 }">
					                ...
					                <tr height="40px"> <td> </td> <td> </td>  <td> </td> <td> </td> <td> </td> <td> </td> <td> </td><td></td> </tr>
					                <tr height="40px"> <td></td> <td></td>  <td></td> <td></td> <td></td> <td></td> <td></td><td></td> </tr>
					                <tr height="40px"><td colspan="9">休息时间</td></tr>
	...
	<script>
		...
		jQuery(document).ready(function() {  
		    ...
		    $("#editCurriculum").on("click",function(){
				...
				if(checkSplitDate(endDay) && checkSplitDate(startDay)){
					standardPost('<%=basePath%>curriculum/toEditCurriculum.do',{semesterId:semesterId,startDay:startDay,endDay:endDay});
				}else{
					layer.msg("请输入正确时间格式！");
				}
			...
			//编辑班级课程表
		    $("#editCurriClours").on("click",function(){
				//学期Id
				var semesterId = $("#semesterId").val();
				//班级Id
		...
		function searchData(){
			...
			//班级Id
			var clazzId = $("#clazzList").val();
			if(checkSplitDate(startDate) && checkSplitDate(endDate)){
		...
		function checkCurriculum()
		{
			...
			if(clazzId==null ||clazzId==''){
				layer.alert("导入前请先选择班级！(该时间段内没有课程表存在)");
				return flag;
			}
			$.ajax({
				type: "post",
				...
				success: function(data)
				{
					...
					{
						layer.alert("该时间段内已有课程表存在，请勿重新导入，如有问题请点编辑进行修改！");
						flag = false;
					}
					else
					{
