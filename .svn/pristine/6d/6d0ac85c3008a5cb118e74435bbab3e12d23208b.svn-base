...
<body>
	...
	<div class="app_head">
		<i class="m_icon-return icon_left" onclick="history.go(-1);"></i>进出宿舍搜索
	</div>
	<div class="app_search">
	
		<form action="<%=basePath%>dormitory/toDormitoryInoutListByHuxinId.do" class="form-horizontal" method="post">
			<c:if test="<%=flag%>">
				<span>师生类型</span>
				<ul id="type">
					<li class="select" id="teacher">教师</li><li id="student">学生</li>
					<input type="hidden" class="val" id="ty" name="type" value="teacher">
				</ul>
			</c:if>
			<span>进出状态</span>
			<ul id="inoutType">
				<li class="select" id="">全部</li><li id="in">进</li><li id="out">出</li>
				<input type="hidden" id="inout" class="val" name="inoutType" value="">
			</ul>
