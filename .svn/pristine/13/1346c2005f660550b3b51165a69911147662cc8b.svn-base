...
//        Wrappeer.append(progressBarTd);
        ...
        var Wrappeer = this.fileProgressWrapper;
        Wrappeer.attr('id', this.fileProgressID).addClass('info');
        
        var fileSize = plupload.formatSize(file.size).toUpperCase();

        var progressBarWrapper = $("<div style=margin-bottom:0px />");
        progressBarWrapper.addClass("progress progress-striped");
        ...
        var progressBarPercent = $('<span class=sr-only style=width:100% />');
        progressBarPercent.text(fileSize);
        progressBar.append(progressBarPercent);
...
//        Wrappeer.append(progressCancel);
        ...
        var progressBarStatus = $('<div class="status text-center"/>');
        Wrappeer.append(progressBarStatus);
...
//    this.fileProgressWrapper.find('.progress-bar-info').attr('aria-valuenow', 0).width('0%').find('span').text('上传完成');
    this.appear();
};
...
FileProgress.prototype.setProgress = function(percentage, speed, chunk_size) {
    ...
    this.fileProgressWrapper.find(".progress-bar-info").css({"background-color":"#9DCA89"});
    var file = this.file;
    var uploaded = file.loaded;

    var size = plupload.formatSize(uploaded).toUpperCase();
    var formatSpeed = plupload.formatSize(speed).toUpperCase();
    var progressbar = this.fileProgressWrapper.find('.progress-bar-info');
//    var progressbar = this.fileProgressWrapper.find('td .progress').find('.progress-bar-info');
    ...
    this.fileProgressWrapper.find('.status').text("已传:" + size + " 速度:" + formatSpeed + "/s");
