...
<body>
	...
	<div class="app_search">
		...
		<table width="100%" class="teachtable" height="100%">
			...
			<tbody>
			<c:if test="${times!=0 }">
				<c:forEach items="${curriculumMorningTimeList}" var="mt"
					varStatus="cmt">
					<tr class="set_bg">
						<td><p>${mt.beginTime}</p>
							<p>-</p>
						...
						<c:choose>
							<c:when
								test="${mt.curriculumList!=null && fn:length(mt.curriculumList) > 1 }">
								<c:forEach items="${mt.curriculumList}" var="ms">
									<td><a href="#select_type" data-toggle="modal"
										onclick="a('${ms.subjectName}','${ms.subjectsUserName}','${ms.classroomName}')">
											<p>
												<b>${ms.subjectName}</b>
											</p>
				...
				<c:forEach items="${curriculumAfternoonTimeList}" var="at"
					varStatus="cat">
					...
					<tr class="set_bg">
						...
						<c:choose>
							<c:when
								test="${at.curriculumList!=null && fn:length(at.curriculumList) > 1 }">
								<c:forEach items="${at.curriculumList}" var="as">
									<td><a href="#select_type" data-toggle="modal"
										onclick="a('${as.subjectName}','${as.subjectsUserName}','${as.classroomName}')">
											<p>
												<b>${as.subjectName}</b>
											</p>
				...
				<c:forEach items="${curriculumNightTimeList}" var="nt"
					varStatus="cnt">
					...
					<tr class="set_bg">
						...
						<c:choose>
							<c:when
								test="${nt.curriculumList!=null && fn:length(nt.curriculumList) > 1 }">
								<c:forEach items="${nt.curriculumList}" var="ns">
									<td><a href="#select_type" data-toggle="modal"
										onclick="a('${ns.subjectName}','${ns.subjectsUserName}','${ns.classroomName}')">
											<p>
												<b>${ns.subjectName}</b>
											</p>
	...
	$(function() { 
		...
		$("#search").on("click",function(){
			...
			standardPost('<%=basePath%>curriculum/toCurriculumListSearchOneAPP.do',{apiKey:apiKey,schoolId:schoolId,roleCode:roleCode})
		})
