...
<body class="page-header-fixed">
	...
	<script>
		...
		//加载老师信息
		function loadTeacherList(clazzId)
		{
			var rowCount = 0;
			$.ajax({
				type: "post",
				url: "<%=basePath%>teacher/selectTeacherListByClazzId.do",
				data:{
					clazzId : clazzId
				},
				success: function(data){
					...
					$.each(studentList, function(n, value) {  
						...
						studentRowHtml += '<h3><a onclick = "toDetail(' + value.id+ ')">' + value.name + '</a></h3>';
						...
						studentRowHtml += '<a onclick = "toDetail(' + value.id+ ')"  class="news-block-btn">';
						studentRowHtml += '查看详情';
		...
		{  
			standardPost('<%=basePath%>teacher/toTeacherDetailView.do',{id:id});
		}
	</script>
