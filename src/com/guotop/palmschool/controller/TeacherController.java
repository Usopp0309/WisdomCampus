...
public class TeacherController extends BaseController
{
	...
	public String doAddTeacher(HttpServletRequest request, HttpSession session, ModelMap modelMap)
	{
		 ...
		 * 是否亦信用户
		 */
		boolean isYinUser = Boolean.parseBoolean(request.getParameter("isYinUser"));
		
		/**
		 * 是否本校
		 */
		boolean isLocalSchool = Boolean.parseBoolean(request.getParameter("isLocalSchool"));
		/**
		 * 参数
		 */
		...
		teacherService.addTeacher(paramMap, teacherBase, teacherDetail, cardCodeSet, roleCode,isYinUser,isLocalSchool);
		return "teacher/teacher_list";
	}
	...
	public String toModifyTeacher(HttpServletRequest request, HttpSession session, ModelMap modelMap)
	{
        ...
        if(StringUtil.isEmpty(fromAdd)){//从添加跳转来的
    		paramMap.put("departmentId", departmentId);	
    		paramMap.put("roleCode", roleCode);
        }
		User teacher = teacherService.getTeacherById(paramMap);
		if(!StringUtil.isEmpty(fromAdd)){//不是从添加跳转来的
	...
	public String doModifyTeacher(HttpServletRequest request, HttpSession session, ModelMap modelMap)
	{
		 ...
		 */
		String[] cardCode = request.getParameterValues("cardCode");
		teacherService.modifyTeacherById(paramMap,cardCode);
		return "teacher/teacher_list";
	}
