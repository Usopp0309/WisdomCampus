 ...
 * TODO 该巡更功能已经改成扫码巡更
 */
public class PatrolTCPServer extends Thread
{

	private static final Log log = LogFactory.getLog(PatrolTCPServer.class);

	private static ServerSocket server = null;

	protected final static int PORT = 8359;
	...
	public PatrolTCPServer()
	{
		if (null == server)
		{
			...
			{
				log.error("启动PatrolTCPServer失败", e);
				e.printStackTrace();
			}
		}
	}
...
//		log.info("卡号 ： " + code);
		...
		PatrolInout inout = new PatrolInout();
//		inout.setCode(code);
		...
		return StartupListener.patrolService.addPatrolInout(inout, ipSwitch,position);
	}
...
//			log.info("Socket[" + this.getName() + "]开始：" + ip);
			...
			if (in != null)
			{
				...
				while (true)
				{
					...
					try
					{
						...
						if (b[0] == 82) 
						{
							...
							if (pos == 21)
							{
								...
								{
									log.error("PatrolTCPServer246行出错，错误信息如下："+ex);
								}
					...
					{
						log.error("PatrolTCPServer264行出错，错误信息如下："+e);
						break;
					}
				}
			}
