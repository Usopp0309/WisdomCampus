...
public class SchoolServiceImpl extends BaseService implements SchoolService
{
	...
	public List<School> getSchoolListByUserId(Integer userId)
	{
		...
		if(1 == userId){
			...
			list = this.getBaseDao().selectListByObject("School.getAllSchool", map);
		}else{
			list = getBaseDao().selectListByObject("School.getSchoolsByUserId", userId);
		}
		return list;
	}
	...
	public Pages getAllSchool(int pageSize, int page, HashMap<String, Object> map) {
		...
		try
		{
			allRow = (Integer) this.getBaseDao().selectObjectByObject("School.getAllSchoolCount", map);
			totalPage = Pages.countTotalPage(pageSize, allRow);
			final int offset = Pages.countOffset(pageSize, page);
			final int length = pageSize;
			currentPage = Pages.countCurrentPage(page);
