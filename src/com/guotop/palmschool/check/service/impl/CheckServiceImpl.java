...
public class CheckServiceImpl extends BaseService implements CheckService
{
	...
	public Pages loadStudentCheckList(int pageSize, int page, Map<String, Object> paramMap, User user)
	{
		...
		else 
		{
			...
			List<Integer> clazzIdList = commonService.getIdsByUserIdAndPermissionCode(user.getUserId(), "studentAttendanceManager", 0);
	...
	public List<StudentCheck> getStudentCheckListForExcel(Map<String, Object> paramMap, User user)
	{
		...
		}else if (roleCode != null && "student".equals(roleCode))//学生
		{
			return getBaseDao().selectListByObject("Check.getStudentCheckListAsStudent", paramMap);
		}else if (roleCode != null && "parent".equals(roleCode))//家长
		{
			return this.getBaseDao().selectListByObject("Check.getStudentCheckListAsParent", paramMap);
		}else
		{
			paramMap.put("userId", user.getUserId());
			paramMap.put("permissionCode", "studentAttendanceManager");
	...
	public CheckAttendance getCheckAttendanceByUserIdAndCreateDateAndStatus(Integer userId, Integer status,String createDate){
		...
		return (CheckAttendance) getBaseDao().selectObjectByObject("Check.getCheckAttendanceByUserIdAndCreateDateAndStatus", paramMap);
	}
	/**
	 * 添加考勤统计表
	 */
	...
	{
		//添加之前先判断是否 存在
		CheckAttendance ca = getCheckAttendanceByUserIdAndCreateDateAndStatus(checkAttendance.getUserId(),checkAttendance.getStatus(),checkAttendance.getCreateDate());
		if(ca != null){
			updateCheckAttendance(checkAttendance.getUserId(),checkAttendance.getStatus(),checkAttendance.getCreateDate());
		}else{
	...
	public void updateCheckAttendance(Integer userId, Integer status,String createDate)
	{
		...
		paramMap.put("status", status);
		paramMap.put("createDate", createDate);
		paramMap.put("remark", "请假");
		getBaseDao().updateObject("Check.updateCheckAttendance", paramMap);
	}
	...
	public List<CheckAttendance> checkAMTeacherAttendance(Map<String, Object> paramMap, User user)
	{
		...
		{
			paramMap.put("userId", user.getUserId());
			paramMap.put("permissionCode", "attendanceStatisticsManager");//考勤统计权限值
			List<Integer> departIdList = getBaseDao().selectListByObject("UserPermission.getOptDepartsByUserIdAndPermissionCode", paramMap);
			paramMap.put("departIdList", departIdList);
	...
	public List<CheckAttendance> checkStudentAttendance(Map<String, Object> paramMap,User user)
	{
		...
		if (commonService.hasAdminPermission(user))
		{
			return getBaseDao().selectListByObject("Check.checkStudentAttendance", paramMap);
		}else//有查询权限
		{
			paramMap.put("userId", user.getUserId());
			paramMap.put("permissionCode", "attendanceStatisticsManager");
			List<Integer> clazzIdList = commonService.getIdsByUserIdAndPermissionCode(user.getUserId(), "attendanceStatisticsManager", 0);
			paramMap.put("clazzIdList", clazzIdList);
			return getBaseDao().selectListByObject("Check.checkStudentAttendanceWithTeacher", paramMap);
	...
	public List<CheckAttendance> checkAMStudentAttendance(Map<String, Object> paramMap,User user){
		...
		if (commonService.hasAdminPermission(user))
		{
			return getBaseDao().selectListByObject("Check.checkAMStudentAttendance", paramMap);
		}else//有查询权限
		{
			paramMap.put("userId", user.getUserId());
			paramMap.put("permissionCode", "attendanceStatisticsManager");//考勤统计权限值
			List<Integer> clazzIdList = commonService.getIdsByUserIdAndPermissionCode(user.getUserId(), "attendanceStatisticsManager", 0);
			paramMap.put("clazzIdList", clazzIdList);
			return getBaseDao().selectListByObject("Check.checkAMStudentAttendanceWithTeacher", paramMap);
		}
	...
	public List<CheckAttendance> checkPMStudentAttendance(Map<String, Object> paramMap,User user){
		...
		if (commonService.hasAdminPermission(user))
		{
			return getBaseDao().selectListByObject("Check.checkPMStudentAttendance", paramMap);
		}else//有查询权限
		{
			paramMap.put("userId", user.getUserId());
			paramMap.put("permissionCode", "attendanceStatisticsManager");//考勤统计权限值
			List<Integer> clazzIdList = commonService.getIdsByUserIdAndPermissionCode(user.getUserId(), "attendanceStatisticsManager", 0);
			paramMap.put("clazzIdList", clazzIdList);
			return getBaseDao().selectListByObject("Check.checkPMStudentAttendanceWithTeacher", paramMap);
		}
