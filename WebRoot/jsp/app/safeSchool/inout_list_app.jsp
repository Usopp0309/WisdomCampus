	...
	<form action="<%=basePath%>inout/toInoutListByHuxinId.do"  id="addForm" class="form-horizontal" method="post">
		<c:if test="${inoutListSize > 0}">
		    ...
		    <c:if test="${inoutListSize >19}">
			       <a id="op_all_save" class="op_all_save" onclick="loadInoutList()">查看更多</a>
		    </c:if>
		</c:if>
		<input type="hidden" id="roleCode" name="roleCode" value="${roleCode}">
		<input type="hidden" id="roleList" name="roleList" value="${roleList}">
	            ...
	            <div class="modal-content">
	                <div class="modal-body">
						<ul class="more_role_choose" id="moreRole">
					    	<li id="0">请选择角色</li>
					    	<c:set value="0" var="adIndex"/>
					    	<c:set value="0" var="preIndex"/>
					    	<c:set value="0" var="depIndex"/>
					    	<c:set value="0" var="LeadIndex"/>
					    	<c:set value="0" var="teacIndex"/>
					    	<c:set value="0" var="stuIndex"/>
					    	<c:set value="0" var="parIndex"/>
					    	<c:forEach items="${roleList}" var="r">
						       <c:if test="${r.roleCode == 'admin' && adIndex==0}" >
							  	 <li id="${r.roleCode}">校管</li>
							  	 <c:set value="1" var="adIndex"/>
						       </c:if>
						       <c:if test="${r.roleCode == 'president' && preIndex==0}">
							  	 <li id="${r.roleCode}">校长</li>
							  	 <c:set value="1" var="preIndex"/>
						       </c:if>
						       <c:if test="${r.roleCode == 'departManager' && depIndex==0}">
							  	 <li id="${r.roleCode}">部门管理员</li>
							  	 <c:set value="1" var="depIndex"/>
						       </c:if>
						       <c:if test="${r.roleCode == 'classLeader' && LeadIndex==0}">
							  	 <li id="${r.roleCode}">班主任</li>
							  	 <c:set value="1" var="LeadIndex"/>
						       </c:if>
						       <c:if test="${r.roleCode == 'teacher' && teacIndex==0}">
							  	 <li id="${r.roleCode}">教师</li>
							  	 <c:set value="1" var="teacIndex"/>
						       </c:if>
						       <c:if test="${r.roleCode == 'student' && stuIndex==0}">
							  	 <li id="${r.roleCode}">学生</li>
							  	 <c:set value="1" var="stuIndex"/>
						       </c:if>
						        <c:if test="${r.roleCode == 'parent' && parIndex==0}">
							  	 <li id="${r.roleCode}">家长</li>
							  	 <c:set value="1" var="parIndex"/>
						       </c:if>
						   </c:forEach>
						</ul>
					</div>
