...
//        uptoken_url: '/aaa',
        ...
        init: {
            'FilesAdded': function(up, files) {
            	   file = files[0];
            	   qiniufileName=file.name;
            	   qiniufileName=qiniufileName.substring(0,qiniufileName.lastIndexOf("."));
	...
	function upload(path)
	{
		...
		var fileSize_=$("#upload .sr-only").text();
		if(fileSize_==""){
			fileSize_="未知";
		}
		...
		$.ajax({
			...
			async: false,
			type:"post",
			data:{"qiniukey":qiniukey_,"fileName":qiniufileName,"fileSize":fileSize_},
			dataType:"json",
			...
			success:function(result){
				...
				location.href=base+"/teachingResources/toAppTeacherResource.do";
			}
		});
		}
	}
